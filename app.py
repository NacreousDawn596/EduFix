    ...
    if "email" in session:
        ...
        if issue:
            return render_template('demandes.html', username=session.get('username'), email=session.get('email'), 
                                   side_panel=side_panel[session['position']], issue=issue, pos=session['pos'] >= 1, technicien = session['pos'] == -1, techniciens = [i for i in techniciens if i['position'] == -1] if session['pos'] == 1 else [i for i in techniciens if i['position'] == -2 and i['at'] == session['dep']], session=session)
        else:
            return redirect(url_for('login'))
    else:
        return redirect(url_for('login'))
